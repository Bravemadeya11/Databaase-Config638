...
DROP PROCEDURE IF EXISTS GetPatientsByPhysicianCriteria $$

CREATE PROCEDURE GetPatientsByPhysicianCriteria(
    IN p_PhysicianID INT,
    IN p_MinTreatments INT,
...
BEGIN
    SELECT 
        ...
        P.ContactDate,
        CASE 
            WHEN P.OutPatientYN = 'Y' AND P.ResidentYN = 'N' THEN 'Outpatient'
            WHEN P.ResidentYN = 'Y' AND P.OutPatientYN = 'N' THEN 'Resident'
            WHEN P.OutPatientYN = 'Y' AND P.ResidentYN = 'Y' THEN 'Both'
            ELSE 'Unknown'
        END AS PatientType
    FROM 
        Patient P
        INNER JOIN Person PR ON P.PatientID = PR.PersonID
        INNER JOIN Physician PH ON P.PhysicianID = PH.PhysicianID
        INNER JOIN Person PHPR ON PH.PhysicianID = PHPR.PersonID
        INNER JOIN (
            SELECT PatientID, COUNT(TreatmentID) AS TreatmentCount
            FROM PatientTreatment
            GROUP BY PatientID
        ) PT ON P.PatientID = PT.PatientID
    WHERE 
        P.PhysicianID = p_PhysicianID
        AND PH.Specialty = p_Specialty
        AND PT.TreatmentCount >= p_MinTreatments
    ORDER BY 
        P.ContactDate ASC;
END $$
...
CALL GetPatientsByPhysicianCriteria(15, 1, 'General');

DELIMITER $$

DROP PROCEDURE IF EXISTS GetPhysicianSpecialties $$

CREATE PROCEDURE GetPhysicianSpecialties(IN PhysicianID INT)
BEGIN
    SELECT 
        P.PersonID,
        CONCAT(P.FirstName, ' ', P.LastName) AS PhysicianFullName,
        GROUP_CONCAT(DISTINCT PH.Specialty ORDER BY PH.Specialty SEPARATOR ', ') AS Specialties
    FROM 
        Person P
        INNER JOIN Physician PH ON P.PersonID = PH.PhysicianID
    WHERE 
...
-- Example usage:
CALL GetPhysicianSpecialties(14);
